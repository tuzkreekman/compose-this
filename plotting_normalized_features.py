...
# This file generates plots for the normalized features used in section 3.1.3
...
ax.set_title('STFT with Normalize');

fig, ax = plt.subplots(figsize=(80,1))
mfcc_data = tmp[:,-bands:].T[:,500:1000]
cax = ax.imshow(mfcc_data, interpolation='nearest', cmap=cm.coolwarm, origin='lower')
ax.figure.colorbar(cax, ax=ax)
ax.set_title('Mel Spectrogram with Normalize');

fig, ax = plt.subplots(figsize=(8,8))
mfcc_data = tmp.T[:,:]
cax = ax.imshow(mfcc_data, interpolation='nearest', cmap=cm.coolwarm, origin='lower')
ax.figure.colorbar(cax, ax=ax)
ax.set_title('STFT + Mel Spectrogram with Normalize');
